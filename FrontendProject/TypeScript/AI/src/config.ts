...
const isDesktop = isBrowser && Boolean(window.desktop);
...
const wsProtocol = !isDesktop && pageProtocol === 'https:' ? 'wss:' : 'ws:';
const httpProtocol = !isDesktop && pageProtocol === 'https:' ? 'https:' : 'http:';

// 本地 Vite 开发时后端使用 8000；部署后默认使用同源 Nginx。
const backendHost = isDesktop
  ? '127.0.0.1:8000'
  : isLocalFrontend
    ? `${pageHostname}:8000`
    : pageHost;
